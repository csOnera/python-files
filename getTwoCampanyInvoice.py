...
# one = openpyxl.load_workbook(r"C:\Users\onera\OneDrive - ONE ERA (HK) LIMITED\批發做單\出入貨單\ONERA 出，入貨 2022報稅.xlsx")
# oneWs = one['operation page']

# oneList = []
# for i in range(2, oneWs.max_row + 1):
#     if oneWs['b'+ str(i)].value != None and re.search('23-+', str(oneWs['b'+ str(i)].value)) == None:
#         oneList.append(oneWs['b'+ str(i)].value)
...
#         csList.append(csWs['b'+ str(i)].value)
...
#         if one == cs:
#             print(one)
#             print(realCsList.index(one))
#             print(realOneList.index(one))

# invoicewb = openpyxl.load_workbook(r"C:\Users\onera\OneDrive - ONE ERA (HK) LIMITED\oneraShare\CHARMSMART 2023 出入貨紀錄.xlsx")
# invoicews = invoicewb.active

# def findDate(row):
#     counter = 1
#     while True:
#         if invoicews['a' + str(row + counter)].value != None:
#             return invoicews['a' + str(row + counter)].value
#         else:
#             counter += 1
...
# for row in range(3, 650):
#     if invoicews['b' + str(row)].value != None:
#         if re.search('cs|onera', str(invoicews['b' + str(row)].value)) == None:
#             dateString = str(findDate(row))
#             # print(invoicews['b' + str(row)].value, invoicews['b' + str(row + 1)].value, invoicews['a' + str(row+1)].value)
#             getdate = re.search('[\d/]+', dateString).group()
#             # turn date form
#             getdate = datetime.datetime.strptime(getdate, "%d/%m/%Y").strftime("%Y-%m-%d")

#             invoiceList[str(invoicews['b' + str(row)].value)] = [invoicews['b' + str(row + 1)].value, getdate]
...
for row in range(2, csw.max_row + 1):
    if type(csw["b"+str(row)].value) == str:
        count += 1
        if re.search("cs|onera", csw["b"+str(row)].value) != None:
            
            if re.search("cs|onera", csw["b"+str(row)].value).group() == "cs":
                csl.append(csw["b"+str(row-1)].value)
            elif re.search("cs|onera", csw["b"+str(row)].value).group() == "onera":
                onel.append(csw["b"+str(row-1)].value)
            else:
                print("error?" + str(row))
...
# combined
# for i in realOneList:
#     print(i)
# for i in onel:
#     print(i)
mixCs = realOneList + onel
